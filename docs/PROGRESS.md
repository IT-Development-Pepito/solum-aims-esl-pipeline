...
- Secret provisioning has no supported tool. The bundle has a reader but no writer, so #79 adds an administrative CLI for setting a secret and for checking connectivity to every configured database on demand. DPAPI scope is decided as **user scope**, which means the bundle must be created by the Windows Service account itself, and an administrator resetting that account's password makes the bundle permanently unreadable. #79 is the only unblocked implementable issue; #78 is blocked behind it.
