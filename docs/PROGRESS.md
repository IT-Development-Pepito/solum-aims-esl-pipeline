...
- **UNKNOWN / NEEDS-DISCOVERY:** Every `end_device_templates` row has `page = 0`, yet the Hop pipeline `esl-sku-promo-multi-page.hpl` is documented as enabling Page 3. The two probably mean different things: `end_device_templates.page` is likely a template slot, while the displayed page may be held in `AIMS_CORE_DB.enddevice.pages`, a `varchar(100)`. Not yet read.
