...
| BR-019 | Promotion reference; review-only `RefreshESL_New` | The reference-directed target requires `PROMO_PRIORITY_DIFFERENT_ECONOMIC` for different calculated outcomes and `DISPLAY_PRIORITY_SAME_ECONOMIC` for equal calculated outcomes with different terms/display. Effective-price/rounding comparison and legacy procedure parity need explicit tests. | VERIFIED within newly supplied reference; review-only procedure compares raw type/price/percent and does not establish the required calculated-outcome parity; winner policy remains NEEDS-DISCOVERY | Promotion-ambiguity rule. |
