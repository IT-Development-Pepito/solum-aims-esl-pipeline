...
| FR-003 | Validate source schema, required fields, key uniqueness, data types, and configured range/domain rules before side effects. | Invalid rows are rejected with a reason and do not reach AIMS actions. |
| FR-004 | Produce deterministic transformations from the same input, rule version, and configuration version. | Repeat test returns identical canonical output/hash. |
...
| FR-006 | Support record-level rejection/quarantine where the workflow can safely continue. | Valid records complete; rejected records are counted, traceable, and replayable only after correction. |
| FR-026 | Support the current two-store operation and additional configured stores in the future without a code change or a separate workflow definition per store. | Integration test processes two configured stores; capacity test demonstrates a third configured store under the documented per-store concurrency policy. |
...
| FR-021 | Reconcile expected source records, eligible records, successes, rejections, submissions, acknowledgements, and unresolved outcomes. | Reconciliation report balances counts and enumerates exceptions. |
| FR-022 | Record who/what/when/why/configuration/input/outcome/retry evidence for every execution and material record action. | Audit query answers the questions in the project brief for a sampled run. |
...
| FR-028 | Retain CSV generation only as a bounded compatibility delivery adapter when an identified external consumer requires it. | The consumer contract, acknowledgement, retention, and decommission gate are documented; CSV output is not treated as completed delivery without its defined acknowledgement. |
...
| BR-005 | `RefreshESL_New` | Promotion fields are populated from current campaigns. | VERIFIED; campaign winner rule NEEDS-DISCOVERY | Promotion eligibility/selection. |
| BR-006 | Troubleshooting report | Price category and overlapping promotion behavior are ambiguous. | VERIFIED | Explicit approved price/promotion policy. |
...
Required evidence to resolve unknown rules: approved merchandising/POS rule source, representative production cases, current business owner confirmation, and parity tests against the accepted baseline. No code may silently treat existing behavior as the desired policy.
