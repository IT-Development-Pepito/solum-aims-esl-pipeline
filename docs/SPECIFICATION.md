...
- **VERIFIED:** The newly supplied `ESL_Promotion_Business_Logic_and_Business_Rules_Reference.md` records the current promotion compatibility baseline: date/time-led eligibility, category-`001` regular price, explicit PFS exclusion, UOM handling, atomic promotion state, and observable ambiguity. It also identifies promotion priority, non-CLR conversion, and a final weekday-metadata policy as unresolved.
- **VERIFIED:** The latest supplied `RefreshESL_New` text contains Patch 2.5 promotion-pipeline changes, but is explicitly marked “REVIEW/TEST ONLY” and contains an apparent direct invocation of `dbo.RefreshESL_New` inside its own procedure body. No successful execution evidence was supplied. Treat it as current implementation evidence requiring SQL review, not as proof of a safe production deployment.
...
| BR-005 | `RefreshESL_New`; promotion reference | Use operational campaign records and current date/time as the primary eligibility input; campaign status and warehouse metadata are supporting evidence, not the primary authority. Cross-midnight windows must be supported. | VERIFIED within newly supplied reference and review-only procedure; production parity remains NEEDS-DISCOVERY | Campaign eligibility rule. |
...
| BR-019 | Promotion reference | Different eligible economic outcomes must emit `PROMO_PRIORITY_DIFFERENT_ECONOMIC`; equal economic outcomes with different campaign terms/display must emit `DISPLAY_PRIORITY_SAME_ECONOMIC`. | VERIFIED within newly supplied reference; winner policy NEEDS-DISCOVERY | Promotion-ambiguity rule. |
| BR-020 | Latest supplied procedure text | The procedure is marked review/test only and contains an apparent direct self-invocation. Its deployment/execution safety is not established by the supplied evidence. | VERIFIED source-text observation | SQL-review gate; no target runtime dependency. |
