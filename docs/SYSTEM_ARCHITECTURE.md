...
The AIMS mutation boundary (#23, `adapters/aims_page.py`) is the only path that changes anything in AIMS, and it speaks HTTP alone: it holds no database engine, so AD-002 is structural rather than a rule to remember. It posts the operation of record named in AD-021 with the caller's #19 idempotency key in an `Idempotency-Key` header, which the vendor does not document and which is therefore ours to send and log; safety does not rest on it, because setting a label to a page is idempotent in effect and the action ledger is authoritative. The adapter reports a typed outcome rather than raising, and its classification turns on one question: did the request arrive? A connection refused, a connect timeout, or a 502/503/504 gateway means it did not, which is `NOT_DELIVERED` and retryable. A read timeout, a cut connection, a 500, or a 200 whose body cannot be parsed means it may already have moved a label, which is `UNKNOWN`, reaches an operator, and is never resent by the runner (FR-013, architecture 5.6). `ACTIVE` mode remains refused at the persist step until one controlled live call settles which of `currentPage`, `returnPage`, and `exceptionPage` the endpoint drives, so the adapter is contract-tested but not yet wired into a submission path.
...
The purge covers record differences, promotion evaluations and candidates, record issues, execution checkpoints and steps, detailed events, and, since #64, record processing results, canonical snapshots, and snapshot sets. Three links pointed from the retained audit core down into those detailed rows, all `RESTRICT`. `0009_relax_action_evidence_links` made two of them optional, `record_action.record_processing_result_id` and `record_processing_result.canonical_record_snapshot_id`; the third, `reconciliation_exception.record_processing_result_id`, was already nullable and needed no migration. The purge releases all three deliberately before deleting what they pinned, and `RESTRICT` stays everywhere, so no delete ever cascades. Nulling costs no audit value: an action retains its store, item, selling UOM, record key, desired state, and idempotency key, and an exception retains its category and both evidence sides, so each remains interpretable without the row it referenced. Purge remains disabled by default, because no retention duration is approved.
